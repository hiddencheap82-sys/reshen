...
final class SmsTemplates
{
    ...
    private const TEMPLATES = [
        ...
        /*
         * تنها الگویی که به *سالن* می‌رود، نه به مشتری.
         *
         * چرا لازم است: نوبتِ اینترنتی برای هفتهٔ بعد، در صفحهٔ «صف
         * زنده» که فقط امروز را نشان می‌دهد دیده نمی‌شود. سالن تا
         * وقتی خودش به «رزروها» سر نزند خبر ندارد — و برای کسی که
         * تازه از دفترچه آمده، این یعنی «معلوم نیست ثبت شده یا نه».
         *
         * critical نیست: نرسیدنش کسی را پشت در نمی‌گذارد، چون نوبت
         * در پنل هست. ولی رسیدنش همان چیزی است که اعتماد می‌سازد.
         */
        'salon_new_booking' => [
            'title' => 'نوبت تازه (به سالن)',
            'vars' => ['name', 'date', 'time'],
            'pattern' => "نوبت تازه: %name%\nتاریخ: %date%\nساعت: %time%",
            'critical' => false,
            'note' => 'به شمارهٔ خودِ سالن می‌رود، وقتی مشتری از اینترنت نوبت می‌گیرد. اگر شمارهٔ سالن در تنظیمات خالی باشد، فرستاده نمی‌شود.',
        ],
        'booking_cancelled' => [
            'title' => 'لغو نوبت',
            'vars' => ['name', 'salon', 'date', 'time'],
