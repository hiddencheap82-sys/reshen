...
<div class="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
  <div class="bg-white rounded-2xl border border-ink-100 p-4 text-center">
    <div class="text-2xl font-extrabold text-gold-700"><?= fa_num($metrics['active_salons']) ?></div>
    <div class="text-[11px] text-ink-400 mt-1">سالن فعال</div>
  </div>
  <div class="bg-white rounded-2xl border border-ink-100 p-4 text-center">
    <div class="text-2xl font-extrabold text-ink-800"><?= fa_num($metrics['completed_this_week']) ?></div>
    <div class="text-[11px] text-ink-400 mt-1">نوبت به‌سرانجام‌رسیدهٔ هفته</div>
  </div>
  <div class="bg-white rounded-2xl border border-ink-100 p-4 text-center">
    ...
    <div class="text-[11px] text-ink-400 mt-1">MAE (دقیقه)</div>
  </div>
  <div class="bg-white rounded-2xl border border-ink-100 p-4 text-center">
    ...
    <div class="text-[11px] text-ink-400 mt-1">نرخ ثبت پایان</div>
  </div>
</div>
...
<div class="bg-white rounded-2xl border border-ink-100 divide-y divide-ink-100">
  ...
  <a href="<?= url('platform/' . $s['id']) ?>" class="flex items-center justify-between px-5 py-3.5 hover:bg-ink-50">
    ...
    <div class="text-left">
      <span class="text-[11px] px-2 py-0.5 rounded-full bg-gold-50 text-gold-700"><?= e($s['plan_code']) ?></span>
      <div class="text-[11px] text-ink-400 mt-1"><?= jdate($s['created_at'], 'Y/m/d') ?></div>
    </div>
  </a>
  <?php endforeach; ?>
</div>
