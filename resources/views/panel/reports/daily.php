...
<div class="flex items-center justify-between mb-5">
  <h1 class="text-lg font-bold text-slate-800">گزارش روزانه</h1>
  <a href="<?= url('panel/reports/monthly') ?>" class="text-xs text-brand-600 hover:underline">گزارش ماهانه ←</a>
</div>

<form method="get" class="mb-5">
  <input type="date" name="date" value="<?= e($date) ?>" onchange="this.form.submit()" class="rounded-xl border border-slate-200 px-3 py-2 text-sm">
</form>
...
<div class="grid md:grid-cols-2 gap-5">
  <div class="bg-white rounded-2xl border border-slate-100 p-5">
    ...
    <?php endforeach; ?>
    <?php if (empty($breakdown)): ?><p class="text-xs text-slate-400">پرداختی ثبت نشده.</p><?php endif; ?>
  </div>
  <div class="bg-white rounded-2xl border border-slate-100 p-5">
    <h2 class="text-sm font-bold text-slate-700 mb-3">به تفکیک آرایشگر</h2>
    <?php foreach ($byStaff as $s): ?>
    <div class="flex items-center justify-between text-sm py-1.5">
      <span class="text-slate-600"><?= e($s['name']) ?> <span class="text-[11px] text-slate-400">(<?= fa_num($s['count']) ?>)</span></span>
      <span class="font-bold text-slate-800"><?= toman((int)$s['total']) ?></span>
    </div>
    <?php endforeach; ?>
  </div>
</div>
